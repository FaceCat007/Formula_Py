...
class BaseShape(object):
	def __init__(self):
		...
		self.m_leftOrRight = TRUE #依附于左轴或右轴
...
class FCChart(FCView):
	def __init__(self):
		super().__init__()
		self.m_allowDragChartDiv = TRUE #是否允许拖拽图层
		...
		self.m_candleMaxRight = 0 #蜡烛线的右轴最大值
		self.m_candleMinRight = 0 #蜡烛线的右轴最小值
		self.m_volMaxRight = 0 #成交量层的右轴最大值
		self.m_volMinRight = 0 #成交量层的右轴最小值
		self.m_indMaxRight = 0 #指标层的右轴最大值
		self.m_indMinRight = 0 #指标层的右轴最小值
		self.m_indMax2Right = 0 #指标层2的右轴最大值
		self.m_indMin2Right = 0 #指标层2的右轴最小值
		...
		self.m_gridColor = "rgb(50,0,0)" #网格颜色 
...
def getChartYInRight(chart, divIndex, value):
	if(divIndex == 0):
		if(chart.m_candleMaxRight > chart.m_candleMinRight):
			cValue = value
			cMax = chart.m_candleMaxRight
			cMin = chart.m_candleMinRight
			if(chart.m_vScaleType != "standard"):
				if (cValue > 0):
					cValue = log10(cValue)
				elif (cValue < 0):
		...
		if(chart.m_volMaxRight > chart.m_volMinRight):
			rate = (value - chart.m_volMinRight) / (chart.m_volMaxRight - chart.m_volMinRight)
			candleHeight = getCandleDivHeight(chart)
			volHeight = getVolDivHeight(chart)
			return candleHeight + volHeight - chart.m_volPaddingBottom - (volHeight - chart.m_volPaddingTop - chart.m_volPaddingBottom) * rate
		else:
		...
		if(chart.m_indMaxRight > chart.m_indMinRight):
			rate = (value - chart.m_indMinRight) / (chart.m_indMaxRight - chart.m_indMinRight)
			candleHeight = getCandleDivHeight(chart)
			volHeight = getVolDivHeight(chart)
			indHeight = getIndDivHeight(chart)
			return candleHeight + volHeight + indHeight - chart.m_indPaddingBottom - (indHeight - chart.m_indPaddingTop - chart.m_indPaddingBottom) * rate
		...
		if(chart.m_indMax2Right > chart.m_indMin2Right):
			rate = (value - chart.m_indMin2Right) / (chart.m_indMax2Right - chart.m_indMin2Right)
			candleHeight = getCandleDivHeight(chart)
			volHeight = getVolDivHeight(chart)
			indHeight = getIndDivHeight(chart)
			indHeight2 = getIndDivHeight2(chart)
...
def calculateChartMaxMin(chart):
	...
	if (isTrend):
		...
		if(chart.m_indMax2 == 0 and chart.m_indMin2 == 0):
			chart.m_indMax2 = 1
			chart.m_indMin2 = -1
		if(chart.m_candleMaxRight == 0 and chart.m_candleMinRight == 0):
			chart.m_candleMaxRight = 1
			chart.m_candleMinRight = -1
		if(chart.m_volMaxRight == 0 and chart.m_volMinRight == 0):
			chart.m_volMaxRight = 1
			chart.m_volMinRight = -1
		if(chart.m_indMaxRight == 0 and chart.m_indMinRight == 0):
			chart.m_indMaxRight = 1
			chart.m_indMinRight = -1
		if(chart.m_indMax2Right == 0 and chart.m_indMin2Right == 0):
...
def mouseMoveChart(chart, firstTouch, secondTouch, firstPoint, secondPoint):
	...
	global m_calculteMaxMin
	global m_firstPaddingTop
	global m_firtstPaddingBottom
	...
	if (firstTouch and secondTouch):
		...
		if(chart.m_allowDragChartDiv):
			chart.m_candlePaddingTop = m_firstPaddingTop - int(m_firstTouchPointCache_Chart.y - firstPoint.y)
			chart.m_candlePaddingBottom = m_firtstPaddingBottom + int(m_firstTouchPointCache_Chart.y - firstPoint.y)
...
def drawChartScale(chart, paint, clipRect):
	if(chart.m_leftVScaleWidth > 0):
		paint.fillRect(chart.m_scaleColor, chart.m_leftVScaleWidth, 0, chart.m_leftVScaleWidth + m_lineWidth_Chart, chart.m_size.cy - chart.m_hScaleHeight)
	if(chart.m_rightVScaleWidth > 0):
		paint.fillRect(chart.m_scaleColor, chart.m_size.cx - chart.m_rightVScaleWidth, 0, chart.m_size.cx - chart.m_rightVScaleWidth + m_lineWidth_Chart, chart.m_size.cy - chart.m_hScaleHeight)
	if(chart.m_hScaleHeight > 0):
		paint.fillRect(chart.m_scaleColor, 0, chart.m_size.cy - chart.m_hScaleHeight, chart.m_size.cx, chart.m_size.cy - chart.m_hScaleHeight + m_lineWidth_Chart)
	...
	if(volDivHeight > 0):
		paint.fillRect(chart.m_scaleColor, chart.m_leftVScaleWidth, candleDivHeight, chart.m_size.cx - chart.m_rightVScaleWidth, candleDivHeight + m_lineWidth_Chart)
	if(indDivHeight > 0):
		paint.fillRect(chart.m_scaleColor, chart.m_leftVScaleWidth, candleDivHeight + volDivHeight, chart.m_size.cx - chart.m_rightVScaleWidth, candleDivHeight + volDivHeight + m_lineWidth_Chart)
	if(indDivHeight2 > 0):
		paint.fillRect(chart.m_scaleColor, chart.m_leftVScaleWidth, candleDivHeight + volDivHeight + indDivHeight, chart.m_size.cx - chart.m_rightVScaleWidth, candleDivHeight + volDivHeight + indDivHeight + m_lineWidth_Chart)
	if(chart.m_data != None and len(chart.m_data) > 0):
		topPoint = FCPoint(0, 10)
		bottomPoint = FCPoint(0, candleDivHeight - 10)
		candleMax = getChartValue(chart, topPoint)
		candleMin = getChartValue(chart, bottomPoint)
		ret = chartGridScale(candleMin, candleMax,  (candleDivHeight - chart.m_candlePaddingTop - chart.m_candlePaddingBottom) / 2, chart.m_vScaleDistance, chart.m_vScaleDistance / 2, int((candleDivHeight - chart.m_candlePaddingTop - chart.m_candlePaddingBottom) / chart.m_vScaleDistance))
		...
		ret = chartGridScale(volMin, volMax,  (volDivHeight - chart.m_volPaddingTop - chart.m_volPaddingBottom) / 2, chart.m_vScaleDistance, chart.m_vScaleDistance / 2, int((volDivHeight - chart.m_volPaddingTop - chart.m_volPaddingBottom) / chart.m_vScaleDistance))
		if(m_gridStep_Chart > 0 and ret > 0):
			start = 0
			if (volMin >= 0):
		...
		if(indDivHeight > 0):
			...
			ret = chartGridScale(indMin, indMax, (indDivHeight - chart.m_indPaddingTop - chart.m_indPaddingBottom) / 2, chart.m_vScaleDistance, chart.m_vScaleDistance / 2, int((indDivHeight - chart.m_indPaddingTop - chart.m_indPaddingBottom) / chart.m_vScaleDistance))
			if(m_gridStep_Chart > 0 and ret > 0):
				start = 0;
				if (indMin >= 0):
		...
		if(indDivHeight2 > 0):
			...
			ret = chartGridScale(indMin2, indMax2, (indDivHeight2 - chart.m_indPaddingTop2 - chart.m_indPaddingBottom2) / 2, chart.m_vScaleDistance, chart.m_vScaleDistance / 2, int((indDivHeight2 - chart.m_indPaddingTop2 - chart.m_indPaddingBottom2) / chart.m_vScaleDistance))
			if(m_gridStep_Chart > 0 and ret > 0):
				start = 0;
				if (indMin2 >= 0):
...
def drawChartCrossLine(chart, paint, clipRect):
	...
	if(chart.m_showCrossLine):
		...
		if(chart.m_sPlot == None and chart.m_selectShape == ""):
			paint.fillRect(chart.m_crossLineColor, chart.m_leftVScaleWidth, drawY, chart.m_size.cx - chart.m_rightVScaleWidth, drawY + m_lineWidth_Chart)
			paint.fillRect(chart.m_crossLineColor, drawX, 0, drawX + m_lineWidth_Chart, chart.m_size.cy - chart.m_hScaleHeight)
...
def drawChartStock(chart, paint, clipRect):
	if (chart.m_data != None and len(chart.m_data) > 0):
		...
		if (isTrend):
			...
			newPaint.drawPolyline(m_indicatorColors[7], m_lineWidth_Chart, 0, drawPoints)
		hasMinTag = FALSE
		hasMaxTag = FALSE
		for i in range(chart.m_firstVisibleIndex,lastValidIndex + 1):
			x = getChartX(chart, i)
			...
			if (close >= openValue):
				if (isTrend == FALSE):
					newPaint.fillRect(chart.m_upColor, x, highY, x + m_lineWidth_Chart, lowY)
					if (cWidth > 0):
						if (close == openValue):
							newPaint.fillRect(chart.m_upColor, x - cWidth, closeY, x + cWidth, closeY + m_lineWidth_Chart)
						else:
							newPaint.fillRect(chart.m_upColor, x - cWidth, closeY, x + cWidth + 1, openY)
			else:
				if (isTrend == FALSE):
					newPaint.fillRect(chart.m_downColor, x, highY, x + m_lineWidth_Chart, lowY)
					if (cWidth > 0):
						newPaint.fillRect(chart.m_downColor, x - cWidth, openY, x + cWidth + 1, closeY)
			if (chart.m_selectShape == "CANDLE"):
				...
				if (i % kPInterval == 0):
					if (isTrend == FALSE):
						newPaint.fillRect(m_indicatorColors[0], x - 3, closeY - 3, x + 3, closeY + 3)
			if (isTrend == FALSE):
				if (hasMaxTag == FALSE):
					if (high == chart.m_candleMax):
						tag = toFixed(high, chart.m_candleDigit)
						tSize = newPaint.textSize(tag, chart.m_font)
						newPaint.drawText(tag, chart.m_textColor, chart.m_font, x - tSize.cx / 2, highY - tSize.cy - 2)
						hasMaxTag = TRUE
				if (hasMinTag == FALSE):
					if (low == chart.m_candleMin):
						tag = toFixed(low, chart.m_candleDigit)
						tSize = newPaint.textSize(tag, chart.m_font)
						newPaint.drawText(tag, chart.m_textColor, chart.m_font, x - tSize.cx / 2, lowY + 2)
						hasMinTag = TRUE
		...
		for i in range(chart.m_firstVisibleIndex,lastValidIndex + 1):
			...
			if (close >= openValue):
				if (isTrend):
					if(volHeight > 0):
						paint.fillRect(m_indicatorColors[6], x, volY, x + m_lineWidth_Chart, zeroY)
				else:
					if (cWidth > 0):
						if(volHeight > 0):
		...
		if (indHeight > 0):
			if (chart.m_showIndicator == "MACD"):
				zeroY = getChartY(chart, 2, 0)
				paint.fillRect(m_indicatorColors[4], chart.m_leftVScaleWidth, zeroY, getChartX(chart, chart.m_lastVisibleIndex), zeroY + m_lineWidth_Chart)
				for i in range(chart.m_firstVisibleIndex,lastValidIndex + 1):
					x = getChartX(chart, i)
		...
		for i in range(0, len(chart.m_shapes)):
			...
			if(shape.m_type == "bar"):
				...
				if(shape.m_leftOrRight):
					...
					if(chart.m_selectShape == shape.m_name):
						drawChartLinesInRight(chart, paint, clipRect, shape.m_divIndex, shape.m_datas, shape.m_color, TRUE)
					else:
						drawChartLinesInRight(chart, paint, clipRect, shape.m_divIndex, shape.m_datas, shape.m_color, FALSE)
